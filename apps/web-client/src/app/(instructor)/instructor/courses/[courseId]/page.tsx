...
const STEP_DEFINITIONS: StepDefinition[] = [
  { id: 1, title: 'Cơ bản', subtitle: 'Tiêu đề, danh mục, thông tin khóa học', icon: <BookOpen className="size-4" /> },
      ...
      <FocusTrap>
        <div role="dialog" aria-modal="true" aria-labelledby="modal-cat-title" className="w-full max-w-md rounded-2xl border border-white/20 bg-white p-6 shadow-2xl">
          <div className="mb-4 flex items-center justify-between">
            ...
            <button onClick={onClose} aria-label="Đóng" className="text-muted-foreground transition-colors hover:text-foreground">
              <X className="size-5" />
            </button>
          </div>
          <div className="space-y-3">
            <div>
              ...
              <Input id="cat-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="VD: Lập trình Web, Marketing..." onKeyDown={(e) => { if (e.key === 'Enter') void handleCreate(); }} autoFocus />
            </div>
